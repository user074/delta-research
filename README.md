...
3. Start your code agent. For full autonomy use `--dangerously-skip-permissions` (Claude Code) or `--yolo` (Codex). If you use Codex, you need to enable multi-agent mode in the config. You can enable this feature from the CLI with /experimental. Enable Multi-agents, then restart Codex.
